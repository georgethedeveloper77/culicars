...
describe('vehicleProcessor', () => {
  ...
  describe('new vehicle (create path)', () => {
    it('creates vehicle when VIN not in DB', async () => {
      ...
      (mockPrisma.vehicle.create as MockInstance).mockResolvedValue({ vin: baseVehicle.vin });
    ...
    it('stores japanAuctionGrade on create', async () => {
      (mockPrisma.vehicle.findUnique as MockInstance).mockResolvedValue(null);
  ...
  describe('existing vehicle (update path)', () => {
    ...
    it('low confidence (listing) does NOT overwrite existing non-null make', async () => {
      ...
      if (updateCall) {
        ...
        expect(mockPrisma.vehicle.update).not.toHaveBeenCalledWith(
          expect.objectContaining({ data: expect.objectContaining({ make: 'Honda' }) })
        );
      }
    });

    it('BE FORWARD grade (0.85) sets japanAuctionGrade when empty', async () => {
      (mockPrisma.vehicle.findUnique as MockInstance).mockResolvedValue(existingVehicle);
      (mockPrisma.vehicle.update as MockInstance).mockResolvedValue({});
