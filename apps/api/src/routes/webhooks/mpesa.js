...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const { CheckoutRequestID: checkoutId, MerchantRequestID: merchantId, ResultCode: resultCode, ResultDesc: resultDesc, CallbackMetadata: metadata, } = callback;
        console.info(`[M-Pesa Webhook] CheckoutID=${checkoutId} Result=${resultCode} Desc=${resultDesc}`);
        if (resultCode === 0) {
            // ── SUCCESS ──
            ...
            await (0, paymentProviderService_1.confirmPayment)(checkoutId, {
            ...
            console.info(`[M-Pesa Webhook] Payment confirmed. Receipt=${metaMap['MpesaReceiptNumber']}`);
        }
        else {
            // ── FAILURE ──
            // Common codes: 1032=cancelled, 1037=timeout, 1=insufficient balance
            await (0, paymentProviderService_1.failPayment)(checkoutId, `ResultCode ${resultCode}: ${resultDesc}`);
            console.info(`[M-Pesa Webhook] Payment failed. Code=${resultCode} Desc=${resultDesc}`);
        }
    }
    catch (err) {
