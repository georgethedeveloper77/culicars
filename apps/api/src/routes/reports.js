...
const reportUnlockService_1 = require("../services/reportUnlockService");
 ...
 */
router.get('/by-vin/:vin', async (req, res, next) => {
    try {
        const parsed = reportValidator_1.getReportByVinSchema.safeParse({ params: req.params });
        if (!parsed.success) {
        ...
        const reportId = await (0, reportService_1.getOrGenerateByVin)(vin);
        // Return full report
        const report = await (0, reportService_1.getFullReport)(reportId, userId);
        if (!report) {
...
router.get('/:id', async (req, res, next) => {
    try {
        const parsed = reportValidator_1.getReportByIdSchema.safeParse({ params: req.params });
        if (!parsed.success) {
            return res.status(400).json({
                error: 'VALIDATION_ERROR',
        ...
        const userId = req.user?.id;
        const report = await (0, reportService_1.getFullReport)(id, userId);
        if (!report) {
            return res.status(404).json({
                error: 'NOT_FOUND',
...
router.get('/:id/preview', async (req, res, next) => {
    try {
        ...
        const { id } = parsed.data.params;
        const userId = req.user?.id;
        const preview = await (0, reportService_1.getReportPreview)(id, userId);
        if (!preview) {
            return res.status(404).json({
                error: 'NOT_FOUND',
                message: 'Report not found',
...
router.post('/:id/unlock', async (req, res, next) => {
    try {
        const parsed = reportValidator_1.unlockReportSchema.safeParse({ params: req.params });
        if (!parsed.success) {
            return res.status(400).json({
                error: 'VALIDATION_ERROR',
                message: parsed.error.issues[0]?.message || 'Invalid report ID',
        ...
        const { id } = parsed.data.params;
        const result = await (0, reportUnlockService_1.unlockReport)(userId, id);
