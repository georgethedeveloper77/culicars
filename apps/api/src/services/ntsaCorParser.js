...
exports.parseCorText = parseCorText;
 ...
 */
function extractRawFields(text) {
    ...
    return {
        ...
        yearOfManufacture: parseYear(findField(lines, fullText, [
            'year of manufacture',
            'year of mfg',
            'year',
            'manufactured',
        ...
        numberOfTransfers: parseTransferCount(findField(lines, fullText, [
            'number of transfers',
            'transfer count',
            'no. of transfers',
            'ownership changes',
 ...
 */
function findField(lines, fullText, keywords) {
...
function sanitizeAndNormalize(raw) {
    const rawResult = raw.registrationNumber
        ? (0, utils_1.normalizePlate)(raw.registrationNumber)
        : null;
...
function parseEngineCC(value) {
    if (!value)
        return null;
    const match = value.match(/(\d{3,5})/);
    return match ? parseInt(match[1], 10) : null;
}
function parseTransferCount(value) {
    if (!value)
        return null;
...
function normalizeInspectionStatus(value) {
    ...
    if (lower.includes('pass') || lower.includes('valid'))
        return 'passed';
...
function normalizeCaveatStatus(value) {
    if (!value)
        return 'unknown';
    const lower = value.toLowerCase();
...
function formatPlateDisplay(normalized) {
    if (!normalized)
        return '';
