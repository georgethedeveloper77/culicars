...
exports.enrichFromCor = enrichFromCor;
 ...
 */
async function enrichFromCor(parsed, consentId, userId) {
    ...
    await prisma_1.default.$transaction(async (tx) => {
        ...
        if (parsed.inspectionDate && parsed.inspectionStatus !== 'unknown') {
            const eventType = parsed.inspectionStatus === 'failed'
                ? 'INSPECTION_FAILED'
                : 'INSPECTED';
 ...
 */
function buildVehicleData(parsed) {
    const data = {
    ...
    if (parsed.yearOfManufacture)
        data.year = parsed.yearOfManufacture;
    if (parsed.engineCapacity)
        data.engineCc = parsed.engineCapacity;
    if (parsed.fuelType)
        data.fuelType = parsed.fuelType;
