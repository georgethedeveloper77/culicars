...
import type { ExtractedChassis } from '../types/ocr.types';
 ...
 */
const CHASSIS_PATTERNS: Array<{ regex: RegExp; baseConfidence: number }> = [
 ...
 */
export function extractChassis(ocrText: string): ExtractedChassis[] {
  if (!ocrText || !ocrText.trim()) return [];

  const results: ExtractedChassis[] = [];
  ...
  for (const segment of segments) {
    for (const { regex, baseConfidence } of CHASSIS_PATTERNS) {
      ...
      while ((match = regex.exec(segment.text)) !== null) {
        ...
        if (normalized.length < 7 || normalized.length > 17) continue;
        ...
        const confidence = Math.min(
          baseConfidence + segment.keywordBoost,
          1.0
        );
 ...
 */
function findChassisSegments(text: string): ChassisSegment[] {
  const keywords = [
