...
import type { ContribType, ContributionSubmission } from '../types/contribution.types.js';

const VALID_CONTRIB_TYPES: ContribType[] = [
  'MILEAGE_RECORD',
  'DAMAGE_REPORT',
  'SERVICE_RECORD',
  'OWNERSHIP_TRANSFER',
...
export function validateContributionSubmission(
  body: Partial<ContributionSubmission>,
): ValidationResult {
  const errors: string[] = [];
  ...
  } else if (!VALID_CONTRIB_TYPES.includes(body.type as ContribType)) {
