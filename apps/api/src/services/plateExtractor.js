...
exports.extractPlates = extractPlates;
 ...
 */
function extractPlates(ocrText) {
    ...
    for (const { regex, baseConfidence } of PLATE_PATTERNS) {
        ...
        while ((match = regex.exec(cleanedText)) !== null) {
            ...
            const result = (0, utils_1.normalizePlate)(raw);
            const normalized = typeof result === 'string' ? result : result?.normalized || '';
            if (!normalized || seen.has(normalized))
                continue;
            seen.add(normalized);
            // Adjust confidence based on context clues
            const confidence = adjustPlateConfidence(baseConfidence, raw, cleanedText, match.index);
            results.push({
                value: normalized,
 ...
 */
function adjustPlateConfidence(base, raw, fullText, matchIndex) {
    let confidence = base;
