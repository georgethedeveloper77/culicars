...
exports.TheftSection = TheftSection;
    ...
    return (<div className="p-6 space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {checks.map(check => {
            ...
            return (<div key={check.db} className={`rounded-xl border p-4 ${style.bg} ${style.border}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {check.date && (<p className="text-xs text-cc-faint mt-1">
                      {new Date(check.date).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </p>)}
                  {check.obNumber && (<p className="text-xs text-cc-muted mt-1">OB: <span className="font-mono">{check.obNumber}</span></p>)}
                </div>
              </div>
            </div>);
        })}
      ...
      {/* Community reports */}
      {communityReports.length > 0 && (<div>
          <p className="text-sm font-semibold text-cc-text mb-3">Community Stolen Reports</p>
          <div className="space-y-3">
            {communityReports.map(rep => (<div key={rep.id} className="cc-card-2 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    : rep.status === 'recovered'
                      ...
                      {rep.isObVerified && (<span className="cc-pill bg-blue-500/10 text-blue-400 border border-blue-500/30 text-xs">
                          🏛️ OB Verified
                        </span>)}
                    </div>
      ...
      <div className="border border-dashed border-cc-border-2 rounded-xl p-4 flex items-center justify-between gap-4">
        ...
        <link_1.default href={data?.plate ? `/report-stolen/${data.plate}` : '/report-stolen'} className="cc-btn-danger shrink-0 text-xs">
          Report Stolen
        </link_1.default>
      </div>
    </div>);
