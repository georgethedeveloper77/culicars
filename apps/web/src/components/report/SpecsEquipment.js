...
exports.SpecsEquipment = SpecsEquipment;
const AUCTION_GRADE_META = {
...
function SpecsEquipment({ data }) {
    ...
    return (<div className="p-6 space-y-6">
      ...
      {grade && gradeStyle && (<div className={`rounded-xl border p-4 flex items-center gap-4 ${gradeStyle.color}`}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {specs.map(s => (<div key={s.label} className="cc-card-2 px-3 py-2.5">
      ...
      {data.optionCodes && data.optionCodes.length > 0 && (<div>
            ...
            {categories.map(cat => (<div key={cat}>
                ...
                <div className="flex flex-wrap gap-2">
                  {data.optionCodes
                    ...
                    .map((opt, i) => (<span key={i} className="cc-pill bg-blue-500/10 text-blue-300 border border-blue-500/20 text-xs" title={opt.code}>
                        {opt.value}
                      </span>))}
                </div>
              </div>))}
