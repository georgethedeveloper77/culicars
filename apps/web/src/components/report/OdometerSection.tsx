...
const CustomDot = (props: { cx?: number; cy?: number; payload?: MileageRecord }) => {
...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: Array<{ payload: MileageRecord }> }) => {
  if (!active || !payload?.length) return null;
  const rec = payload[0].payload;
  return (
    <div className="cc-card text-xs p-3 shadow-xl">
...
export function OdometerSection({ data }: { data: OdometerSectionData }) {
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, bottom: 10, left: 10 }}>
                ...
                <XAxis
                  dataKey="date"
                  tickFormatter={d => new Date(d).toLocaleDateString('en-KE', { year: '2-digit', month: 'short' })}
                ...
                {rollbackIndices.map(idx => (
                  <ReferenceLine
                    key={idx}
                    x={chartData[idx]?.date}
                    stroke="#ef4444"
                    strokeDasharray="4 2"
                    label={{ value: '↓ Rollback', fill: '#ef4444', fontSize: 9 }}
                  />
                ))}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-cc-border/50">
                {records
                  ...
                  .map((rec, i) => (
                    <tr key={i} className={rec.isRollback ? 'bg-red-950/20' : ''}>
                      ...
                      <td className="py-2">
                        {rec.isRollback ? (
                          <span className="cc-pill bg-red-600/10 text-red-400 border border-red-600/30 text-xs">
                            ⚠ Rollback
                          </span>
                        ) : (
                          <span className="text-cc-faint text-xs">—</span>
                        )}
                      </td>
                    </tr>
                  ))}
