'use client';
// apps/web/src/components/report/DamageSection.tsx

interface DamageIncident {
  id: string;
  location: string;       // e.g. "Front right / Bumper"
  zone: string;           // "front" | "rear" | "left" | "right" | "roof" | "underbody"
  severity: 'damage' | 'severe_damage';
  costRangeMin: number;
  costRangeMax: number;
  date?: string;
  county?: string;
...
function CarSilhouette({ incidents }: { incidents: DamageIncident[] }) {
  return (
    <svg viewBox="0 0 300 500" className="w-full max-w-[180px] mx-auto" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        return (
          <g key={inc.id || i}>
            ...
            <circle
              ...
              cy={pos.cy}
              r={isSevere ? 18 : 14}
              fill={isSevere ? 'rgba(239,68,68,0.15)' : 'rgba(234,179,8,0.15)'}
            />
            {/* Main dot */}
            <circle
              cx={pos.cx}
...
export function DamageSection({ data }: { data: DamageSectionData }) {
    ...
    <div className="p-6">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-3">
          {incidents.map((inc, i) => (
            <div key={inc.id || i} className="cc-card-2 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  ...
                  <span className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${
                    inc.severity === 'severe_damage' ? 'bg-red-600' : 'bg-amber-500'
                  }`}>
                    {i + 1}
                  </span>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className={`cc-pill text-xs ${
                        inc.severity === 'severe_damage'
                          ? 'bg-red-600/10 text-red-400 border border-red-600/30'
                          : 'bg-amber-500/10 text-amber-400 border border-amber-500/30'
                      }`}>
                        {inc.severity === 'severe_damage' ? '⚠ Severe damage' : '▲ Damage'}
                      </span>
                      {inc.possibleCause && inc.possibleCause !== 'Unknown' && (
                        <span className="cc-pill bg-cc-surface text-cc-muted text-xs">{inc.possibleCause}</span>
                      )}
                    </div>
                    {inc.severity === 'severe_damage' && (
