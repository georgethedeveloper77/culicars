...
import { recordNtsaConsent } from '@/lib/api';

interface NtsaFetchSectionProps {
  vin: string;
  plate: string;
...
export function NtsaFetchSection({ vin, plate, ntsaVerified, onConsented }: NtsaFetchSectionProps) {
  const { user, token } = useAuth();
  const [consented, setConsented] = useState(false);
  const [loading, setLoading] = useState(false);
  const [step, setStep] = useState<'idle' | 'consented' | 'opened'>('idle');

  const handleConsent = async () => {
    if (!token) return;
    setLoading(true);
    try {
      await recordNtsaConsent({ vin, plate }, token);
      setConsented(true);
      setStep('consented');
      onConsented?.();
    } catch {
    ...
    <div className="p-6">
      <div className="cc-card-2 p-5 space-y-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          {['Consent', 'Open eCitizen', 'Auto-fetch PDF'].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <span className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${
                (i === 0 && step !== 'idle') || (i === 1 && step === 'opened') || (i === 2 && step === 'opened')
        ...
        {/* CTAs */}
        {!user ? (
          <p className="text-sm text-cc-muted">Sign in to fetch NTSA data for this vehicle.</p>
        ) : step === 'idle' ? (
          <button
