...
export function PhotoGrid({ data }: { data: PhotoSectionData }) {
          ...
          <div className="grid grid-cols-3 gap-2">
            {group.photos.map((url, i) => (
              <button
                ...
                className="aspect-video rounded-lg overflow-hidden bg-cc-surface-2 hover:opacity-90 transition-opacity focus:outline-none focus:ring-2 focus:ring-cc-accent"
              >
                <img
                  src={url}
                  alt={`Vehicle photo from ${group.date}`}
