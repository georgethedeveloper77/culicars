...
function ServiceSection({ data }) {
    ...
    return (<div className="p-6 space-y-3">
      {records
            ...
            .map((rec, i) => (<div key={i} className="cc-card-2 p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className="text-sm font-semibold text-cc-text">{rec.garageName}</span>
                  {rec.county && (<span className="cc-pill bg-cc-surface text-cc-muted text-xs">📍 {rec.county}</span>)}
                </div>

                <p className="text-sm text-cc-muted mt-1 leading-relaxed">{rec.workDone}</p>

                {/* Work type tags */}
                {rec.workTypes && rec.workTypes.length > 0 && (<div className="flex flex-wrap gap-1.5 mt-2">
                    {rec.workTypes.map(wt => (<span key={wt} className={`cc-pill text-xs border ${WORK_COLORS[wt] || 'bg-cc-surface text-cc-muted border-cc-border'}`}>
                        {wt}
                      </span>))}
                  </div>)}
              </div>
              ...
              <div className="shrink-0 text-right">
                ...
                {rec.mileageAtService && (<p className="text-xs text-cc-faint mt-0.5">{rec.mileageAtService.toLocaleString()} km</p>)}
