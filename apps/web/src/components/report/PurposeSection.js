...
exports.PurposeSection = PurposeSection;
...
function PurposeSection({ data }) {
    ...
    return (<div className="p-6">
      {foundCount > 0 && (<div className="mb-4 flex items-center gap-2">
          <span className="text-amber-400 text-sm">▲</span>
          <p className="text-amber-400 text-sm font-medium">
            {foundCount} commercial use record{foundCount > 1 ? 's' : ''} found
          </p>
        </div>)}
      {data?.summary && (<p className="text-cc-muted text-sm mb-4">{data.summary}</p>)}

      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {checks.map(check => (<div key={check.type} className={`rounded-xl border p-4 flex flex-col gap-2 transition-colors ${check.found
            ...
            {check.found ? (<span className="flex items-center gap-1 text-amber-400 text-xs font-medium">
                <span>▲</span> Record found
              </span>) : (<span className="flex items-center gap-1 text-cc-faint text-xs">
                <span>✓</span> No record found
              </span>)}
            {check.detail && (<p className="text-xs text-cc-muted leading-tight">{check.detail}</p>)}
          </div>))}
      </div>
