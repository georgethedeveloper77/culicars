// apps/web/src/components/report/ReportCover.tsx
import { Report, ReportSection } from '@/lib/api';
import { RiskBadge } from './RiskBadge';
import { SectionIconCard } from './SectionIconCard';
import { COVER_SECTION_CARDS } from '@/lib/constants';
...
function getSectionStatus(sections: ReportSection[], type: string) {
  const s = sections.find(s => s.sectionType === type);
  if (!s) return 'not_checked' as const;
  return s.dataStatus === 'found' ? 'found' : s.dataStatus === 'not_found' ? 'not_found' : 'not_checked';
}

function isSectionLocked(sections: ReportSection[], type: string) {
  const s = sections.find(s => s.sectionType === type);
  return s?.isLocked ?? true;
}

export function ReportCover({ report, onSectionClick }: ReportCoverProps) {
  ...
  const photosData = report.sections
    ...
    ?.data as { urls?: string[]; groups?: { photos?: string[] }[] } | undefined;
    ...
    <div className="cc-card overflow-hidden">
      ...
      <div className="relative h-48 sm:h-64 bg-cc-surface-2 flex items-center justify-center overflow-hidden">
        {coverPhoto ? (
          ...
          <div className="flex flex-col items-center gap-2 text-cc-faint">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-5 gap-2 mt-6">
          {COVER_SECTION_CARDS.map(card => (
            <SectionIconCard
              key={card.key}
              icon={card.icon}
              label={card.label}
              status={getSectionStatus(report.sections, card.key)}
              isLocked={isSectionLocked(report.sections, card.key)}
              onClick={() => onSectionClick?.(card.key)}
            />
          ))}
        </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className={`cc-pill text-xs font-semibold ${
            report.recommendation === 'proceed' ? 'bg-emerald-500/10 text-emerald-400' :
