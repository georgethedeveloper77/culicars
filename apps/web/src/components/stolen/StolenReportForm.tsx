...
import { submitStolenReport, StolenReportSubmit } from '@/lib/api';
import { KENYA_COUNTIES } from '@/lib/constants';

interface StolenReportFormProps {
  defaultPlate?: string;
  onSuccess: (id: string) => void;
}

export function StolenReportForm({ defaultPlate = '', onSuccess }: StolenReportFormProps) {
  const [form, setForm] = useState<Partial<StolenReportSubmit>>({
  ...
  const update = (field: keyof StolenReportSubmit, value: string) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      const result = await submitStolenReport(form as StolenReportSubmit);
      onSuccess(result.id);
    } catch (err: unknown) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {error && (
        <div className="bg-red-950/30 border border-red-600/40 rounded-xl p-4 text-red-400 text-sm">
          {error}
        </div>
      )}
      ...
      <div className="cc-card p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              className="cc-input font-mono tracking-widest uppercase"
              value={form.plate || ''}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Silver, White, Blue"
              required
            />
          </div>
          <div>
            ...
            <input
              ...
              placeholder="Stickers, damage, custom parts…"
            />
          </div>
        </div>
      </div>
      ...
      <div className="cc-card p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="date"
              className="cc-input"
              value={form.dateStolenIso || ''}
              onChange={e => update('dateStolenIso', e.target.value)}
          ...
          <div>
            ...
            <select
              className="cc-input"
              value={form.countyStolen || ''}
              onChange={e => update('countyStolen', e.target.value)}
              required
            >
        ...
        <div>
          ...
          <input
            className="cc-input"
            value={form.townStolen || ''}
            onChange={e => update('townStolen', e.target.value)}
            placeholder="e.g. Westlands, Mombasa CBD, Industrial Area"
            required
          />
        </div>
      </div>
      ...
      <div className="cc-card p-6 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              className="cc-input font-mono"
              value={form.policeObNumber || ''}
              onChange={e => update('policeObNumber', e.target.value)}
              placeholder="OB/1234/2024"
            />
          </div>
          <div>
            ...
            <input
              ...
              placeholder="e.g. Parklands Police Station"
            />
          </div>
        </div>
      </div>
      ...
      <div className="cc-card p-6 space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {(['owner', 'family', 'witness', 'police'] as const).map(type => (
              <button
                key={type}
                type="button"
                onClick={() => update('reporterType', type)}
                className={`py-2.5 px-3 rounded-lg border text-sm font-medium transition-colors capitalize ${
      ...
      <button
        type="submit"
        disabled={loading}
        className="cc-btn-danger w-full py-4 text-base"
      >
        {loading ? (
