...
interface StolenReportSuccessProps {
  plate: string;
  reportId: string;
}

export function StolenReportSuccess({ plate, reportId }: StolenReportSuccessProps) {
    ...
    <div className="max-w-md mx-auto text-center space-y-6 py-8">
      <div className="cc-card p-8">
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href={`/search?q=${encodeURIComponent(plate)}`} className="cc-btn-secondary w-full">
            Search this plate
          </Link>
          <Link href="/" className="text-sm text-cc-muted hover:text-cc-text transition-colors">
            Back to home
          </Link>
        </div>
      </div>
