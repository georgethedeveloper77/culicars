...
export default function ContributeVinPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!type || !title) { setError('Please select a type and add a title.'); return; }
    setLoading(true);
    setError(null);
    try {
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        <div className="cc-card p-8">
          ...
          <Link href={`/search?q=${vin}`} className="cc-btn-secondary w-full">
            View vehicle report →
          </Link>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {CONTRIBUTION_TYPES.map(ct => (
              <button
                ...
                className={`py-3 px-3 rounded-xl border text-sm font-medium transition-colors flex flex-col items-center gap-1.5 ${
                  type === ct.value
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Oil change at Toyota Kenya — 85,000 km"
            required
          />
        </div>

        {/* Mileage if relevant */}
        {(type === 'MILEAGE_RECORD' || type === 'SERVICE_RECORD') && (
          <div>
