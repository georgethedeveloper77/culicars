...
function HomePage() {
    return (<>
      ...
      <section className="relative overflow-hidden noise-overlay pt-16 pb-20 md:pt-24 md:pb-28">
        {/* Background grid */}
        <div className="absolute inset-0 bg-grid-pattern bg-grid-40 pointer-events-none opacity-100" aria-hidden/>
        {/* Radial glow */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(ellipse at center, rgba(245,166,35,0.08) 0%, transparent 70%)' }} aria-hidden/>
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="mt-8 max-w-xl mx-auto">
            <SearchBar_1.SearchBar size="large" autoFocus/>
          </div>

          {/* Quick example plates */}
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <span className="text-xs text-cc-faint mr-1">Try:</span>
            {RECENT_SEARCHES.map(v => (<link_1.default key={v.plate} href={`/search?q=${encodeURIComponent(v.plate)}`} className="cc-pill bg-cc-surface border border-cc-border text-cc-muted hover:border-cc-accent/40 hover:text-cc-text transition-colors text-xs">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {HOW_IT_WORKS.map(step => (<div key={step.step} className="cc-card p-6 relative overflow-hidden group hover:border-cc-accent/30 transition-colors">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        <div className="relative rounded-2xl overflow-hidden border border-red-900/40 bg-gradient-to-br from-red-950/40 to-cc-surface p-8 md:p-10">
          ...
          <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <link_1.default href="/report-stolen" className="cc-btn-danger shrink-0 whitespace-nowrap">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {DATA_SOURCES.map(src => (<div key={src.name} className="cc-card-2 p-4 flex items-center gap-3">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
        ...
        ].map(item => (<div key={item.title} className="cc-card p-5 flex gap-4">
              <span className="text-2xl">{item.icon}</span>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="font-medium text-cc-text text-sm">{item.title}</h4>
                  {item.free && (<span className="cc-pill bg-emerald-500/10 text-emerald-400 text-xs">Free</span>)}
                </div>
        ...
        <div className="text-center mt-8">
          <link_1.default href="/pricing" className="cc-btn-secondary">View Pricing →</link_1.default>
        </div>
      </section>
    </>);
}
