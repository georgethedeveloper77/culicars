...
import { searchVehicles, SearchResponse, SearchCandidate } from '@/lib/api';
...
function SearchResults() {
  ...
  if (!q) {
    return (
      <div className="text-center py-20 text-cc-muted">
        <p>Enter a plate or VIN to search.</p>
        <Link href="/" className="cc-btn-secondary mt-4 inline-flex">← Back to home</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Error */}
      {error && !loading && (
        <div className="cc-card border-red-900/40 p-6 text-center">
      ...
      {result && !loading && (
        <div className="fade-in space-y-4">
          ...
          {(result.stolenAlert.active || result.stolenAlert.hasActiveReport) && (
            <StolenReportBanner 
  alert={{
...
/>
          ...
          {result.candidates.length === 0 && (
            <div className="cc-card p-8 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link href="/contribute" className="cc-btn-secondary text-sm">
                  Contribute data for this vehicle
                </Link>
                <Link href="/report-stolen" className="cc-btn-secondary text-sm">
                  Report it stolen
                </Link>
              </div>
            </div>
          ...
          {result.candidates.map(c => (
            <CandidateCard key={c.vin} candidate={c} />
          ))}

          {/* No stolen alert — show clean badge */}
          {!result.stolenAlert.active && result.candidates.length > 0 && (
...
function CandidateCard({ candidate: c }: { candidate: SearchCandidate }) {
  const v = c.vehicle;

  return (
    <div className="cc-card p-5 hover:border-cc-accent/30 transition-colors group">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap mb-2">
            ...
            {v?.ntsaCorVerified && (
              <span className="cc-pill bg-blue-500/10 text-blue-400 text-xs border border-blue-500/20">
                🏛️ NTSA Verified
              </span>
            )}
          </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            <div className="flex-1 h-1 bg-cc-border rounded-full max-w-[120px]">
              <div
                className="h-1 rounded-full bg-cc-accent transition-all"
        ...
        <div className="shrink-0 flex flex-col gap-2">
          {c.reportId ? (
            <Link href={`/report/${c.reportId}`} className="cc-btn-primary">
              View Report →
            </Link>
          ) : (
            <Link
              href={`/report/new?vin=${encodeURIComponent(c.vin)}&plate=${encodeURIComponent(c.plate ?? "")}`}
          ...
          <Link
            href={`/contribute/${c.vin}`}
            className="text-xs text-center text-cc-muted hover:text-cc-text transition-colors"
          >
            Contribute data
          </Link>
        </div>
