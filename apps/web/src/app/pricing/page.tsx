      ...
      <div className="mb-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {CREDIT_PACKS.map(pack => (
            <div
              key={pack.id}
              className={`cc-card p-6 flex flex-col items-center text-center relative ${
                (pack as any).popular ? 'border-cc-accent/40' : ''
              }`}
            >
              {(pack as any).popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 cc-pill bg-cc-accent text-cc-bg text-xs font-bold px-3 py-1">
                  Most Popular
                </span>
              )}
              {'dealerPack' in pack && pack.dealerPack && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 cc-pill bg-blue-500 text-white text-xs font-bold px-3 py-1">
                  Dealer Pack
                </span>
              )}
              ...
              <Link href={`/signup?pack=${pack.id}`} className={`w-full text-center text-sm ${(pack as any).popular ? 'cc-btn-primary' : 'cc-btn-secondary'}`}>
      ...
      <div className="cc-card p-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {[
            ...
            <div key={pm.name} className="flex items-center gap-1.5 text-sm text-cc-muted">
              <span>{pm.icon}</span>
              <span>{pm.name}</span>
      ...
      <div className="mt-14">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={faq.q} className="cc-card p-5">
