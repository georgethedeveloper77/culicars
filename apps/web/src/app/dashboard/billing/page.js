...
exports.default = DashboardBillingPage;
...
function DashboardBillingPage() {
    ...
    const [selectedPack, setSelectedPack] = (0, react_1.useState)(null);
    const [selectedProvider, setSelectedProvider] = (0, react_1.useState)('mpesa');
    const [mpesaPhone, setMpesaPhone] = (0, react_1.useState)('');
    const [payStep, setPayStep] = (0, react_1.useState)('idle');
    const [paymentId, setPaymentId] = (0, react_1.useState)(null);
    const [payError, setPayError] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        if (!token)
            return;
        Promise.all([
            (0, api_1.getWalletBalance)(token),
            (0, api_1.getCreditLedger)(token),
            (0, api_1.getPaymentProviders)(),
        ]).then(([bal, led, prov]) => {
            ...
            if (enabled.length > 0)
                setSelectedProvider(enabled[0].slug);
        }).catch(console.error)
            .finally(() => setLoading(false));
    }, [token]);
    const pack = constants_1.CREDIT_PACKS.find(p => p.id === selectedPack);
    const handleInitiatePayment = async () => {
        if (!token || !selectedPack || !selectedProvider)
            return;
        ...
        try {
            const result = await (0, api_1.initiatePayment)({
                packId: selectedPack,
            ...
            const poll = async () => {
                ...
                attempts++;
                try {
                    const status = await (0, api_1.getPaymentStatus)(result.paymentId, token);
                    if (status.status === 'success') {
                        setPayStep('success');
                        // Refresh balance
                        const bal = await (0, api_1.getWalletBalance)(token);
                        setBalance(bal.balance);
                    }
    ...
    return (<div className="space-y-6 fade-in">
      <div className="flex items-center justify-between">
        ...
        {balance !== null && (<div className="cc-pill bg-cc-accent/10 border border-cc-accent/30 text-cc-accent font-semibold">
      ...
      <div className="cc-card p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {constants_1.CREDIT_PACKS.map(p => (<button key={p.id} onClick={() => setSelectedPack(p.id)} className={`relative rounded-xl border p-4 text-center transition-all ${selectedPack === p.id
                ? 'border-cc-accent bg-cc-accent/10'
                : 'border-cc-border bg-cc-surface-2 hover:border-cc-border-2'}`}>
              {p.popular && (<span className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-cc-accent text-cc-bg text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                  Popular
                </span>)}
        ...
        {selectedPack && (<div className="space-y-4 pt-2 border-t border-cc-border">
            <div>
              <p className="cc-label mb-3">Payment method</p>
              <div className="flex flex-wrap gap-2">
                {providers.map(pv => (<button key={pv.slug} onClick={() => setSelectedProvider(pv.slug)} className={`px-4 py-2 rounded-lg border text-sm font-medium transition-colors ${selectedProvider === pv.slug
                    ? 'border-cc-accent bg-cc-accent/10 text-cc-accent'
                    : 'border-cc-border bg-cc-surface-2 text-cc-muted hover:border-cc-border-2'}`}>
                    {pv.name}
                  </button>))}
                {providers.length === 0 && (<p className="text-cc-muted text-sm">Loading payment methods…</p>)}
              </div>
            </div>

            {/* M-Pesa phone */}
            {selectedProvider === 'mpesa' && (<div>
                <label className="cc-label">M-Pesa phone number</label>
                <input className="cc-input max-w-xs" value={mpesaPhone} onChange={e => setMpesaPhone(e.target.value)} placeholder="+254 7XX XXX XXX" type="tel"/>
            ...
            {/* Pay button / status */}
            {payStep === 'idle' && (<button onClick={handleInitiatePayment} className="cc-btn-primary">
                Pay KSh {pack?.priceKes.toLocaleString()} for {pack?.credits} credit{pack && pack.credits > 1 ? 's' : ''}
              </button>)}

            {payStep === 'awaiting_mpesa' && (<div className="flex items-center gap-3 text-cc-muted text-sm">
            ...
            {payStep === 'polling' && (<div className="flex items-center gap-3 text-cc-muted text-sm">
                <svg className="animate-spin h-5 w-5 text-cc-accent" viewBox="0 0 24 24" fill="none">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                </svg>
            ...
            {payStep === 'success' && (<div className="space-y-3">
            ...
            {payStep === 'failed' && (<div className="space-y-2">
      ...
      <div className="cc-card p-6">
        ...
        {loading ? (<div className="space-y-2">
            {[1, 2, 3].map(i => (<div key={i} className="h-10 bg-cc-surface-2 rounded animate-pulse"/>))}
          </div>) : ledger.length === 0 ? (<p className="text-cc-muted text-sm text-center py-6">No transactions yet.</p>) : (<div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-cc-border/50">
                ...
                return (<tr key={entry.id}>
                      ...
                      <td className="py-2.5 pr-4">
                        ...
                        {entry.source && (<span className="text-cc-faint text-xs ml-2">· {entry.source}</span>)}
                      </td>
                      <td className={`py-2.5 pr-4 text-right font-mono font-semibold ${entry.creditsDelta > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
