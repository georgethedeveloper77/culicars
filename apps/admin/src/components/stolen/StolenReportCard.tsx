...
import { Shield, MapPin, Calendar, Phone, Mail } from 'lucide-react';
import { StolenStatusBadge } from '@/components/ui/StatusBadge';
import type { StolenReport } from '@/types/admin.types';

interface StolenReportCardProps {
  report: StolenReport;
}

export function StolenReportCard({ report }: StolenReportCardProps) {
  return (
    <div className="rounded-xl border border-white/8 bg-[#141414] p-5">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          {report.vin && (
            <p className="text-xs font-mono text-zinc-500">{report.vin}</p>
          )}
        </div>
        <StolenStatusBadge status={report.status} />
      </div>
