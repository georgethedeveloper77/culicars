...
function SettingsPage() {
    ...
    async function saveSetting(s) {
        ...
        try {
            let parsed;
            try {
                parsed = JSON.parse(getValue(s));
            }
            catch {
                parsed = getValue(s);
            }
            await (0, api_1.apiPost)(`/admin/settings`, { key: s.key, value: parsed });
            setSettings((prev) => prev.map((p) => (p.key === s.key ? { ...p, value: parsed } : p)));
            setSaved(s.key);
    ...
    if (loading) {
        return (<div>
        <PageHeader_1.PageHeader title="Settings"/>
    ...
    return (<div>
      <PageHeader_1.PageHeader title="Admin Settings" description="Platform-wide configuration stored in admin_settings table"/>

      <div className="space-y-3">
        {settings.map((s) => (<div key={s.key} className="rounded-xl border border-white/8 bg-[#141414] p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-3">
                  <input value={getValue(s)} onChange={(e) => setEdited((prev) => ({ ...prev, [s.key]: e.target.value }))} className="w-full bg-[#0E0E0E] border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-200 font-mono focus:outline-none focus:border-[#D4A843]/50"/>
                </div>
              </div>
              <button onClick={() => saveSetting(s)} disabled={saving === s.key} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/6 hover:bg-white/10 text-sm font-medium transition-colors border border-white/10 flex-shrink-0 mt-5 disabled:opacity-40">
                {saving === s.key ? (<lucide_react_1.RefreshCw className="w-4 h-4 animate-spin"/>) : saved === s.key ? (<span className="text-emerald-400">Saved ✓</span>) : (<>
        ...
        {settings.length === 0 && (<div className="rounded-xl border border-white/8 p-12 text-center">
