...
exports.default = DataSourceDetailPage;
...
function DataSourceDetailPage() {
    ...
    const [runResult, setRunResult] = (0, react_1.useState)(null);
    ...
    return (<div className="p-6 space-y-6 max-w-3xl">
      {/* Back */}
      <link_1.default href="/data-sources" className="text-sm link link-hover text-base-content/60">
        ← Data Sources
      </link_1.default>
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button className="btn btn-primary btn-sm" onClick={handleRun} disabled={running || !source.enabled}>
      ...
      {/* Run result banner */}
      {runResult && (<div className={`alert ${runResult.startsWith('✓') ? 'alert-success' : 'alert-error'} py-2 text-sm`}>
          {runResult}
        </div>)}
      ...
      {/* Edit form */}
      {editing && (<div className="card bg-base-200 p-5 space-y-4">
          <h2 className="font-semibold text-sm">Edit Source</h2>
          <div className="grid grid-cols-2 gap-3">
            <label className="space-y-1">
              <span className="text-xs text-base-content/60">Name</span>
              <input className="input input-bordered input-sm w-full" value={form.name ?? ''} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}/>
            </label>
            <label className="space-y-1">
              <span className="text-xs text-base-content/60">Type</span>
              <input className="input input-bordered input-sm w-full" value={form.type ?? ''} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}/>
            </label>
            <label className="space-y-1">
              <span className="text-xs text-base-content/60">Parser type</span>
              <input className="input input-bordered input-sm w-full" value={form.parser_type ?? ''} onChange={e => setForm(f => ({ ...f, parser_type: e.target.value }))}/>
            </label>
            <label className="space-y-1">
              <span className="text-xs text-base-content/60">Schedule (cron)</span>
              <input className="input input-bordered input-sm w-full" placeholder="e.g. 0 2 * * *" value={form.schedule ?? ''} onChange={e => setForm(f => ({ ...f, schedule: e.target.value }))}/>
            </label>
          </div>
          ...
          <label className="space-y-1 block">
            ...
            <textarea className="textarea textarea-bordered text-xs w-full font-mono" rows={4} placeholder={'{\n  "api_key": "...",\n  "base_url": "..."\n}'} value={form.credentials ?? ''} onChange={e => setForm(f => ({ ...f, credentials: e.target.value }))}/>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" className="toggle toggle-sm toggle-success" checked={form.enabled ?? true} onChange={e => setForm(f => ({ ...f, enabled: e.target.checked }))}/>
          ...
          <div className="flex gap-2">
            ...
            <button className="btn btn-error btn-outline btn-sm ml-auto" onClick={handleDelete} disabled={deleting}>
      ...
      {!editing && (<div className="card bg-base-200 p-5 text-sm space-y-2">
